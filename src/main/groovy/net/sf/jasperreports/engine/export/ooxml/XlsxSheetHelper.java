...
/* !!!ATTANTION!!!
 * This file is parched version of the same class in JasperReport 6.4 library,
 * all changes that were done in this file are marked with comment //Sergey K
 */
...
public class XlsxSheetHelper extends BaseHelper {
	//Sergey K : added constant
	public static final String PROPERTY_MAX_ROW_HEIGHT = "net.sf.jasperreports.export.xls.max.row.height";
...
//			colsWriter.write("<col min=\"" + (colIndex + 1) + "\" max=\"" + (colIndex + 1) + "\" customWidth=\"1\"" + (autoFit ? " bestFit=\"1\"" : " width=\"" + (3f * colWidth / 18f) + "\"")+"/>\n");
	...
	public void exportRow(int rowHeight, Cut yCut, XlsRowLevelInfo levelInfo) 
	{
		...
		//Sergey K : this fix provide opportumity to add max excel row height by adding parameter "net.sf.jasperreports.export.xls.max.row.height"
		try{
			if (isAutoFit && this.jasperReportsContext.getProperty("net.sf.jasperreports.export.xls.max.row.height") != null) {
				int maxRowHeight = Integer.parseInt(this.jasperReportsContext.getProperty(PROPERTY_MAX_ROW_HEIGHT));
				if (maxRowHeight > 0) {
					isAutoFit = false;
