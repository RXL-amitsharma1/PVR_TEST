...
class ScimUserRepositoryImpl implements ScimResourceRepository<ScimUser> {
    ...
    synchronized ScimUser save(ScimUser scimUser) {
        ...
        user.addToTenants(Tenant.load(defaultTenantId))
        ...
        scimUser.groups.each {
            UserGroup userGroup = UserGroup.findByScimId(it.id)
            if (userGroup)
                UserGroupUser.create(userGroup, user, false)
        }
        CRUDService.save(user, [flush: true, failOnError: true])
        return getFrom(user)
    }
    ...
    ScimUser update(ScimUser scimUser) {
        ...
        user.lock()
        copyUser(scimUser, user)
        UserGroupUser.removeAll(user)
        scimUser.groups.each {
            UserGroup userGroup = UserGroup.findByScimId(it.id)
            if (userGroup)
                UserGroupUser.create(userGroup, user, false)
        }
        CRUDService.update(user, [flush: true, failOnError: true])
        return getFrom(user)
    }
            ...
            switch (it.op?.toLowerCase()) {
                case 'replace':
                    if (!it.path) {
                        ...
                        UserGroup userGroup = UserGroup.findByScimIdAndIsDeleted(it.value, false)
                        if (userGroup && !UserGroupUser.exists(userGroup.id, user.id)) {
                            UserGroupUser.create(userGroup, user, false)
                        }
                ...
                case 'add':
                    if (it.path.startsWith('emails') && it.path.endsWith('.value')) {
                        ...
                        it.value?.each { obj ->
                            UserGroup userGroup = UserGroup.findByScimIdAndIsDeleted(obj.value, false)
                            if (userGroup && !UserGroupUser.exists(userGroup.id, user.id)) {
                                UserGroupUser.create(userGroup, user, false)
                            }
                        }
                ...
                case 'remove':
                    if (it.path.startsWith('emails')) {
                        ...
                        it.value?.each { obj ->
                            UserGroup userGroup = UserGroup.findByScimId(obj.value)
    ...
    private ScimUser getFrom(User user, String excludedAttributes = null) {
        ...
        List<UserGroupUser> userGroups = UserGroupUser.findAllByUser(user)
        scimUser.with {
            ...
            groups = userGroups*.userGroup.findAll { !it.isDeleted }.collect {
