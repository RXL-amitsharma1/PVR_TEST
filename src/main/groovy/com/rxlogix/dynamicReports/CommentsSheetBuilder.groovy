...
class CommentsSheetBuilder {
    ...
    public void createReportCommentsSheet(ExecutedReportConfiguration executedConfigurationInstance,Map params,
                               ExecutedTemplateQuery executedTemplateQuery,
                               ArrayList<JasperReportBuilderEntry> jasperReportBuilderEntryList) {
        if (dynamicReportService.isInPrintMode(params) && executedConfigurationInstance?.comments) {
            ReportBuilder reportBuilder = new ReportBuilder()
            JasperReportBuilder commentsSheet = reportBuilder.initializeNewReport()
    ...
    public void createSectionCommentsSheet(ExecutedReportConfiguration executedConfigurationInstance, ReportResult reportResult, Map params,
                                           ExecutedTemplateQuery executedTemplateQuery,
                                           ArrayList<JasperReportBuilderEntry> jasperReportBuilderEntryList) {
        if (dynamicReportService.isInPrintMode(params) && reportResult?.comments) {
            ReportBuilder reportBuilder = new ReportBuilder()
            JasperReportBuilder commentsSheet = reportBuilder.initializeNewReport()
