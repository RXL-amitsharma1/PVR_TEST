...
class ChartUtil {
    ...
    private static void buildMultiLvStrRef(CTMultiLvlStrRef ctMultiLvStrRef, ChartDataSource<?> dataSource) {
        if (dataSource.hasFormulaString()) {
            // in multilevel category ref to datatable breaks formatting - so making it static, uncomment if reference should work
    ...
    public static ifChartInverted(def options) {
        options.chart.inverted == true && options.chart.type == Constants.CHART_TYPE
    }

    public static List<Map> reverseCategories(List categories) {
        categories.each {
            if (it instanceof Map && it.categories) it.categories = reverseCategories(it.categories)
        }
