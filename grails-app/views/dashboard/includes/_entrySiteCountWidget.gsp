...
<div class="grid-stack-item-content rx-widget panel">
    <div class="panel-heading pv-sec-heading rx-widget-header">
        <a href="javascript:void(0)" class="rxmain-container-header-label rx-widget-title"
           name="entrySiteCountTitle${index}"
           data-url="${createLink(controller: 'quality', action: 'redirectFromWidget')}">
            <g:message code="app.widget.button.quality.entrySiteCount.label"/>
        </a>
        <g:render template="includes/widgetButtons" model="[index: index, widget: widget]"/>
    </div>

    <div class="row rx-widget-content nicescroll">
        <div class="siteCount${index}">
            <span class="siteCountWidgetTitle${index}"><span
                    class="scTitleContent">${message(code: "app.widget.reportRequest.no.title")}</span>&nbsp;<span
                    class="fa fa-edit rrTitleIcon"></span></span>
            <span class="siteCountWidgetSearchForm${index}" style="display: none">
                <div class="alert alert-danger alert-dismissible forceLineWrap errorDiv" role="alert" hidden="hidden">
                    <button type="button" class="close" name="siteCountCloseButton${index}">
                ...
                <div><input class="form-control" width="100%" maxlength="255" name="scTitle"
                            placeholder="${message(code: "placeholder.templateQuery.title")}"></div>

                <div class="row">
                    <div class="col-sm-12">
                        <b><g:message code="qualityModule.manualAdd.errorType.label"/>:</b>
                        <span class="checkbox checkbox-primary siteCountWidget${index}">
                ...
                <button class="btn btn-primary saveSiteCountWidget${index}">
                    ${message(code: "default.button.save.label")}
                </button>
                <button class="btn btn-primary siteCountWidgetHideButton${index}">
        ...
        <div class="row rx-widget-content">
            ...
            <div class="quality-chart-container " id="entrySiteCount${index}" style="width: 100%;  position: relative;height: calc(100% - 25px);!important;"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function () {
        let chartData;
        $(function () {
            $('[name="siteCountCloseButton${index}"]').on('click', function () {
                var $container = $(".siteCountWidgetSearchForm${index}");
        ...
        $(function () {
            $('[name="entrySiteCountTitle${index}"]').on('click', function () {
        ...
        function loadFilter() {
            setCheckboxes();
            $(".siteCountWidget${index} input").on('click', function () {
        ...
        function setCheckboxes() {
            ...
            if (settingsString) {
                ...
                var $container = $(".siteCountWidgetSearchForm${index}");
                if (settings.title) {
                    $container.find('input[name=scTitle]').val(encodeToHTML(settings.title));
                    $('.siteCountWidgetTitle${index} .scTitleContent').html(encodeToHTML(settings.title));
                }
        ...
        $(document).on("click", ".interval${index}", function () {
            ...
            selectedType = type;
            $('#entrySiteCount${index} .highcharts-container').hide();
        ...
        function loadData(fromDate, toDate, qualityDataType) {
            ...
            $.ajax({
                "url": entrySiteCountUrl + (fromToDate ? (fromToDate + "&dataType=" + qualityDataType) : "?dataType=" + qualityDataType),
                "dataType": 'json'
            }).done(function (data) {
                chartData = data;
                renderChart();
        ...
        function renderChart() {
            $('#entrySiteCount${index} .highcharts-container').show();
            Highcharts.chart('entrySiteCount${index}', {
                chart: {
                    type: 'bar'
                },
                title: {
                ...
                xAxis: {
                    ...
                    title: {
                        text: '<g:message code="app.widget.button.quality.site.label"/>'
                    }
                },
                yAxis: {
                    min: 0,
        ...
        $(".siteCountWidgetTitle${index}").on('click', function () {
            $(".siteCountWidgetSearchForm${index}").show();
            $(".siteCountWidgetTitle${index}").hide();
            $(this).closest(".rx-widget-content").find(".dataTables_length").parent().show();
        });

        $(".siteCountWidgetHideButton${index}").on('click', function () {
            $(".siteCountWidgetSearchForm${index}").hide();
            $(".siteCountWidgetTitle${index}").show();
            $(this).closest(".rx-widget-content").find(".dataTables_length").parent().hide();
        });

        $(".saveSiteCountWidget${index}").on('click', function () {
            ...
            $('#siteCount${index} .highcharts-container').hide();
        });

        function getCurrentSettings() {
            var settings = {};
            $(".siteCountWidgetSearchForm${index}").find("input").each(function (i) {
                $this = $(this);
                if ($this.attr("name") === "scTitle") {
