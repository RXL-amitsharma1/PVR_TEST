...
<div class="grid-stack-item-content rx-widget panel">
    <div class="panel-heading pv-sec-heading rx-widget-header">
        <a href="javascript:void(0)" class="rxmain-container-header-label rx-widget-title"
           name="productCountTitle${index}"
           data-url="${createLink(controller: 'quality', action: 'redirectFromWidget')}">
            <g:message code="app.widget.button.quality.productCount.label"/>
        </a>
        <g:render template="includes/widgetButtons" model="[index: index, widget: widget]"/>
    </div>

    <div class="row rx-widget-content nicescroll">
        <div class="productCount${index}">
            <span class="productCountWidgetTitle${index}"><span
                    class="pcTitleContent">${message(code: "app.widget.reportRequest.no.title")}</span><span
                    class="fa fa-edit rrTitleIcon"></span></span>
            <span class="productCountWidgetSearchForm${index}" style="display: none">
                <div class="alert alert-danger alert-dismissible forceLineWrap errorDiv" role="alert" hidden="hidden">
                    <button type="button" class="close" name="productCountCloseButton${index}"
                ...
                <div><input class="form-control" width="100%" name="pcTitle" maxlength="255"
                            placeholder="${message(code: "placeholder.templateQuery.title")}"></div>

                <div class="row">
                    <div class="col-sm-12">
                        <b><g:message code="qualityModule.manualAdd.errorType.label"/>:</b>
                        <span class="checkbox checkbox-primary productCountWidget${index}">
                ...
                <button class="btn btn-primary saveProductCountWidget${index}">
                    ${message(code: "default.button.save.label")}
                </button>
                <button class="btn btn-primary productCountWidgetHideButton${index}">
        ...
        <div class="row rx-widget-content">
            <div class="btn-group-sm" data-toggle="buttons">
                ...
                <div class="quality-chart-container " id="productCount${index}" style="width: 100%;  position: relative;height: calc(100% - 25px);!important;"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function () {
        let chartData;
        $(function () {
            $('[name="productCountCloseButton${index}"]').on('click', function () {
                var $container = $(".productCountWidgetSearchForm${index}");
        ...
        $(function () {
            $('[name="productCountTitle${index}"]').on('click', function () {
        ...
        function loadFilter() {
            setCheckboxes();
            $(".productCountWidget${index} input").on('click', function () {
        ...
        function setCheckboxes() {
            ...
            if (settingsString) {
                ...
                var $container = $(".productCountWidgetSearchForm${index}");
                if (settings.title) {
                    $container.find('input[name=pcTitle]').val(encodeToHTML(settings.title));
                    $('.productCountWidgetTitle${index} .pcTitleContent').html(encodeToHTML(settings.title));
                }
        ...
        $(document).on("click", ".interval${index}", function () {
            ...
            selectedType = type;
            $('#productCount${index} .highcharts-container').hide();
        ...
        function loadData(fromDate, toDate, qualityDataType) {
            ...
            $.ajax({
                "url": productCountUrl + (fromToDate ? (fromToDate + "&dataType=" + qualityDataType) : "?dataType=" + qualityDataType),
                "dataType": 'json'
            }).done(function (data) {
                chartData = data;
                renderChart();
        ...
        function renderChart() {
            $('#productCount${index} .highcharts-container').show();
            Highcharts.chart('productCount${index}', {
                chart: {
                    type: 'bar'
                },
                title: {
                ...
                xAxis: {
                    ...
                    title: {
                        text: '<g:message code="app.widget.button.quality.product.label"/>'
                    },
                    labels: {
                        formatter: function () {
        ...
        $(".productCountWidgetTitle${index}").on('click', function () {
            $(".productCountWidgetSearchForm${index}").show();
            $(".productCountWidgetTitle${index}").hide();
            $(this).closest(".rx-widget-content").find(".dataTables_length").parent().show();
        });

        $(".productCountWidgetHideButton${index}").on('click', function () {
            $(".productCountWidgetSearchForm${index}").hide();
            $(".productCountWidgetTitle${index}").show();
            $(this).closest(".rx-widget-content").find(".dataTables_length").parent().hide();
        });

        $(".saveProductCountWidget${index}").on('click', function () {
        ...
        function getCurrentSettings() {
            var settings = {};
            $(".productCountWidgetSearchForm${index}").find("input").each(function (i) {
                $this = $(this);
                if ($this.attr("name") === "pcTitle") {
