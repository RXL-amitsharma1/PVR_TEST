<%@ page import="com.rxlogix.util.ViewHelper" %>
    ...
    <g:if test="${!instance.emailConfiguration?.isDeleted && instance.emailConfiguration?.to}">
        <div class="row">
            <div class="col-xs-12">
                <label><g:message code="app.label.emailConfiguration.to"/></label>
                <g:each in="${instance.emailConfiguration?.to?.split(',')}">
                    <div>${it}</div>
                </g:each>
            </div>
        </div>
