...
<html>
<head>
    <title><g:message code="app.pvc.actionPlan.title"/></title>
...
<div class="content ">
    <div class="container ">
        <rx:container title="${message(code: "app.actionPlan.actionPlan")}" options="${false}">
            <g:render template="/central/includes/actionPlan" model="[module: 'central']"/>
        </rx:container>
        <g:render template="/actionItem/includes/actionItemModal" model="${[categories: ViewHelper.actionItemCategoryForActionPlan()]}"/>
    </div>
</div>
<g:render template="/central/includes/actionPlanCaseListModal"/>
