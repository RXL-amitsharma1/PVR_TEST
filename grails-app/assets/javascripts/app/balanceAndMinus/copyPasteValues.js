    ...
    var initialize = function () {
        ...
        $(document).on("copyAndPaste:paste", function (evt, data) {
            ...
            if (data != null && !_.isEmpty(data)) {
                if (delimiter) {
                    $($toAddContainer).find(caseNumFilter).attr("copyAndPasteWithDelimiter", "true");
                } else {
                    $($toAddContainer).find(caseNumFilter).attr("copyAndPasteWithDelimiter", "false");
                }
                $($toAddContainer).find(caseNumFilter).val(data).trigger('change');
            }
        });
        ...
        copyAndPasteDialog.on('show.bs.modal', function (evt) {
            ...
            var selectedField = '';
            var value = "";
            $(container).closest('.validate-copy-paste').removeAttr('disabled');
            $(document).find('#config-filter-panel').attr("id", "config-filter-panel-fixed");
            if ($($toAddContainer).find(caseNumFilter).attr("copyAndPasteWithDelimiter") === "true") {
                resetDelimiterToSemicolon($($toAddContainer).find(caseNumFilter));
            } else {
                resetDelimiterToNone(copyAndPasteDialog);
            }
            value = $($toAddContainer).find(caseNumFilter).val();
...
//As the icon for modal where we add the case number used at 4 places, so fetching the closest event we are using this method.
function copyPasteClick(evt) {
    $toAddContainer = $(evt).closest('div .toAddContainer');
}