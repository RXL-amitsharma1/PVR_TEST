...
var ColumnView = (function () {
    ...
    function ColumnView(el, options) {
        ...
        // bound functions
        onKeydown = this._onKeydown.on(this);
        onKeyup = this._onKeyup.on(this);
        resize = debounce(this._resize.on(this), 300);
        this._onColumnChangeBound = this._onColumnChange.on(this);
        // onResize = _.on(this._onResize, this);
...
//            }
        ...
        _clearAll: function clearAll(dictionaryLevel) {
            ...
            $('#mainContent').on("columnView.reset." + dictionaryLevel, function (event) {
                ...
                clearAdditionalFilters(dictionaryLevel);
...
//            this.lastColEl = this.columns()[this.columns().indexOf(column) + 1];
            ...
            var index = _.indexOf(_.pluck(columnClass.items, 'className'), "dicLi selected selectedBackground");
            if(index < 0){
                index = _.indexOf(_.pluck(columnClass.items, 'className'), "dicLi highlighted selected selectedBackground");
            }
            var lang = columnClass.models[index].lang;

            this.callbacks.source(value, appendIfValueIsSame, columnClass.dictionaryLevel, that.dictionaryType, true,isCtrlPressed, lang);

            function appendIfValueIsSame(data, level) {
                if (that.value !== value) return;
                if (columnClass.dictionaryLevel == 3 && that.dictionaryType == STUDY_DICTIONARY) {
                    level = level - 1
                }
                that.currentColEl = that.columns()[level - 1];
                that.nextColEl = that.columns()[level];

                if ((columnClass.dictionaryLevel && columnClass.dictionaryLevel <= 3) && that.dictionaryType == PRODUCT_DICTIONARY && that.nextColEl.querySelector(".highlighted")) {
                    return;
                }

                if ((columnClass.dictionaryLevel && columnClass.dictionaryLevel <= 5) && that.dictionaryType == EVENT_DICTIONARY && that.nextColEl.querySelector(".highlighted")) {
                    return;
                }
                ...
                if (level < 6) {
                    if (that.nextColEl && that.nextColEl.querySelector(".highlighted")) { // in search path
                        ...
                        that.callbacks.source(newData.selectedPath, appendIfValueIsSame, parseInt(level) + 1, that.dictionaryType, false, isCtrlPressed, data.lang);
                ...
                sourceParents(value, level, showParents, that.dictionaryType, data.lang);
            }
            ...
            function showParents(data, level) {
                ...
                if (data && level > 1) {
                    ...
                    if (that.currentColEl.querySelector(".highlighted") && that.preColEl && that.preColEl.querySelector(".highlighted")) {
                        ...
                        if (level == 2) {
                            if (selectedDictionaryValue.primarySOC) {
                                ...
                                if (data.items[socIndex] !== undefined) {
                        ...
                        if (level == 3) {
                            if (selectedDictionaryValue.primaryHLGT) {
                                ...
                                if (data.items[hlgtIndex] !== undefined) {
                        ...
                        if (level == 4) {
                            if (selectedDictionaryValue.primaryHLT) {
                                ...
                                if (data.items[hltIndex] !== undefined) {
                    ...
                    if (that.preColEl && (that.preColEl.querySelector(".highlighted") || !that.preColEl.querySelector("li"))) {
                        ...
                        sourceParents(idList, level - 1, showParents, that.dictionaryType, data.lang);
                    }
                }
            }
...
//            }
        ...
        _inputSearch: function inputSearch() {
            ...
            function addListenerToInput(inputList) {
                var objectsToBind = $(inputList).filter(function (index) {
                    ...
                    $(objectsToBind[i]).on(
                        {
                            ...
                            focusout: function () {
                                searchDictionary(this, objectsToBind);
                            }
                        });
                }
            }
            function searchDictionary(selectedInput, inputList) {
                ...
                if (searchTerm.trim() != '') {
                    ...
                    var ref_level = selectedInput.getAttribute('ref_level');
                    var additionalCriteriaData = [];
                    $.each(inputList, function (index, element) {
                        if ($(element).hasClass("additionalCriteria")) {
                            additionalCriteriaData.push({
                    ...
                    that.currentColEl = that.columns()[level - 1];
                    forSearch(searchTerm, level, ref_level, additionalCriteriaData, showSearchResult, that.dictionaryType, selectedInput);

                    if (ref_level != null) {
                        that.currentColEl = that.columns()[ref_level - 1];
                    }
                }
            }
...
ColumnView.prototype.CustomSelect = (function () {
    ...
    function CustomSelect(parent, data, isCtrlPressed) {
        ...
        this.boundOnClick = this._onClick.on(this);
    ...
    CustomSelect.prototype = {
        ...
        _renderItems: function renderItems(container, models, oldItem) {
            ...
            //This is to filter out all the duplicate items based on ID and Name
            var duplicateItemIds = models.map(function(item) {
                return (item.id + item.name)
            }).filter(function(value, index, self){
                return (self.indexOf(value) !== index )
            });

            //This is to avoid the confusion due to same names in both English and Japanese
            var concatLangMap = {
                'en': " (J)",
                'ja': " (E)"
            };
            ...
            models.forEach(function (model) {
                var isDuplicate = (duplicateItemIds.indexOf(model.id + model.name) >= 0) && (model.lang !== userLocale);
                model.name = isDuplicate ? model.name.concat(concatLangMap[userLocale]) : model.name;

                if (existingItems.indexOf(model.id.toString()) == -1) {
        ...
        _select: function select(el, _ctrlPressed, _shiftPressed) {
            var oldValue;
            ...
            } else if (_shiftPressed && this._selectedEl) {

                var found = false;
                var up = [], down = [], currentUp = el, currentDown = el;
                while ((currentUp && currentUp.tagName == 'LI') || (currentDown && currentDown.tagName == 'LI')) {
                    if (currentUp && currentUp.tagName == 'LI') {
                        up.push(currentUp);
                        currentUp = currentUp.previousSibling;
                        if (currentUp == this._selectedEl) {
                            for(var i=0;i<up.length;i++){
                                up[i].classList.add("selected");
                            }
                            break;
                        }
                    }

                    if (currentDown && currentDown.tagName == 'LI') {
                        down.push(currentDown);
                        currentDown = currentDown.nextSibling;
                        if (currentDown == this._selectedEl) {
                            for(var i=0;i<down.length;i++){
                                down[i].classList.add("selected");
                            }
                            break;
                        }
                    }
        ...
        _onClick: function onClick(e) {
            ...
            var _shiftPressed =  e.shiftKey;

            if (e.target.tagName == "LI") {
                e.preventDefault();
                this._select(e.target, _ctrlPressed,_shiftPressed);
            }
        },
