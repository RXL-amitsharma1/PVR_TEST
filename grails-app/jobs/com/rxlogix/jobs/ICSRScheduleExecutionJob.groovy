...
class ICSRScheduleExecutionJob {
    ...
    static triggers = {
        cron name: 'ICSRScheduleExecutionJob', startDelay: 300000, cronExpression: Holders.config.getProperty('icsr.schedule.execution.cron.schedule', '0 0/2 * * * ? *'), misfireInstruction: CronTrigger.MISFIRE_INSTRUCTION_DO_NOTHING
        // each hour
    }
    ...
    def execute() {
        ...
        try {
            reportExecutorService.runICSRScheduledConfigurations()
        } catch (Exception e) {
            log.error("Exception in ICSRScheduleExecutionJob: ${e.message}")
            try {
