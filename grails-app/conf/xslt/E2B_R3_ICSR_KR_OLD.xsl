		...
		<xsl:if test="string-length(reportercommentkor) > 0 or string-length(narrativekor) > 0 or string-length(sendercommentkor) > 0" >
          ...
          <value xsi:type="ED" language="{summaryandreportercommentslang}" mediaType="text/plain">
            <xsl:value-of select="concat(narrativekor,sendercommentkor,reportercommentkor,$newline)" />
					...
					<value xsi:type="ED" language="{summaryandreportercommentslang}" mediaType="text/plain">
						<xsl:value-of select="substring(concat($narrativekorFinal,$sendercommentkorFinal,$reportercommentkorFinal),1,100000)" />
					</value>
					<author typeCode="AUT">
						<assignedEntity classCode="ASSIGNED">
							<code code="{$Reporter}" codeSystem="{$oidAssignedEntityRoleCode}" codeSystemVersion="{$ichoidAssignedEntityRoleCodeVersion}"/>
	...
	<xsl:template match="drugindications" mode="EMA-drug-indication">
		<xsl:if test="string-length(drugindicationprimarysource) > 0 or string-length(drugindicationmeddracode) > 0 or string-length(drugindicationmeddraversion) > 0">
			...
			<inboundRelationship typeCode="RSON">
				<observation moodCode="EVN" classCode="OBS">
					<code code="{$Indication}" codeSystem="{$oidObservationCode}" codeSystemVersion="{$ichObservationCLVersion}"/>
					<xsl:choose>
					...
					<performer typeCode="PRF">
						<assignedEntity classCode="ASSIGNED">
							<code code="{$SourceReporter}" codeSystem="{$oidAssignedEntityRoleCode}" codeSystemVersion="{$ichoidAssignedEntityRoleCodeVersion}"/>
						</assignedEntity>
					</performer>
