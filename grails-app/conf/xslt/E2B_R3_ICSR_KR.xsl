...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				...
				xmlns="urn:hl7-org:v3" xmlns:mif="urn:hl7-org:v3/mif" xmlns:xalan="http://xml.apache.org/xslt">
...
-->
	...
	<xsl:variable name="OIdKRSubstanceDomestic">2.16.840.1.113883.3.989.5.1.10.2.2</xsl:variable>

	<xsl:variable name="OIdKRSubstanceForeign">2.16.840.1.113883.6.294</xsl:variable>
...
<xsl:comment>H.5.r: Case Summary and Reporter’s Comments in Native Language - (<xsl:value-of select="$positionCaseSumNar"/>)</xsl:comment>
	...
	<xsl:template match="summary/casesummarynarrative">
		<xsl:if test="string-length(reportercommentkor) > 0 or string-length(narrativekor) > 0 or string-length(sendercommentkor) > 0">
			<xsl:variable name="positionCaseSumNar">
				<xsl:value-of select="position()"/>
			</xsl:variable>
			...
			<xsl:comment>H.5.r: Case Summary and Reporter’s Comments in Native Language - (<xsl:value-of
					select="$positionCaseSumNar"/>)
			</xsl:comment>
			<component typeCode="COMP">
				<observationEvent moodCode="EVN" classCode="OBS">
          ...
          <value xsi:type="ED" language="{summaryandreportercommentslang}" mediaType="text/plain">
            <xsl:value-of select="concat(narrativekor,sendercommentkor,reportercommentkor,$newline)" />
					...
					<xsl:comment>H.5.r.1a: Case Summary and Reporter's Comments Text</xsl:comment>
					<xsl:comment>H.5.r.1b: Case Summary and Reporter's Comments Language</xsl:comment>
					<xsl:variable name="narrativekorFinal">
						<xsl:choose>
							<xsl:when test="string-length(narrativekor) > 0">
								<xsl:value-of select="concat('Company Narrative:',narrativekor,$newline)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
					...
					<xsl:variable name="sendercommentkorFinal">
						<xsl:choose>
							<xsl:when test="string-length(sendercommentkor) > 0">
								<xsl:value-of select="concat('Sender comments:',sendercommentkor,$newline)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
					...
					<xsl:variable name="reportercommentkorFinal">
						<xsl:choose>
							<xsl:when test='string-length(reportercommentkor) > 0'>
								<xsl:value-of select="concat('Reporter comments:',reportercommentkor)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
					...
					<value xsi:type="ED" language="{summaryandreportercommentslang}" mediaType="text/plain">
						<xsl:value-of
								select="substring(concat($narrativekorFinal,$sendercommentkorFinal,$reportercommentkorFinal),1,100000)"/>
					</value>
					<author typeCode="AUT">
						<assignedEntity classCode="ASSIGNED">
	...
	<!-- H.5.r Case Summary and Reporter’s Comments in Native Language (repeat as necessary) -->
	<xsl:template match="summary/casesummarynarrativebackup">
...
-->
	...
	<xsl:template match="summary/narrativeincludeclinical">
		<xsl:if test="string-length(.) > 0">
			...
			<xsl:comment>H.1: Case Narrative Including Clinical Course, Therapeutic Measures, Outcome and Additional
...
-->
	...
	<xsl:template match="primarysource">
		...
		<xsl:comment>C.2.r: Primary Source(s) of Information - (<xsl:value-of select="$positionPriSrc"/>)
		</xsl:comment>
		<outboundRelationship typeCode="SPRT">
											...
											<xsl:choose>
												...
												<xsl:otherwise>
													<code code="{qualification}" codeSystem="{$oidQualification}"
														  codeSystemVersion="{$emaoidC2r4CLVersion}"/>
												</xsl:otherwise>
											</xsl:choose>
											<xsl:if test="string-length(krmedicalprofessionals) > 0">
												<xsl:comment>C.2.r.4.KR.1: Other medical professionals</xsl:comment>
												<code code="{krmedicalprofessionals}" codeSystem="{$oidKRQualification}" codeSystemVersion="1.0"/>
											</xsl:if>
										</asQualifiedEntity>
									</xsl:if>
				...
				<component typeCode="COMP">
					<observationEvent moodCode="EVN" classCode="OBS">
						...
						<value language="{summaryandreportercommentslang}" xsi:type="ED" mediaType="text/plain">
							<xsl:value-of select="normalize-space(summaryandreportercomments)"/>
						</value>
						<author typeCode="AUT">
							<assignedEntity classCode="ASSIGNED">
								<code code="{$Reporter}" codeSystem="{$oidAssignedEntityRoleCode}"/>
						...
						<assignedPerson classCode="PSN" determinerCode="INSTANCE">
							...
							<xsl:if test="string-length(krsendertype)> 0">
								<asQualifiedEntity classCode="QUAL">
									...
									<code code="{krsendertype}" codeSystem="{$OidKRSenderType}" codeSystemVersion="1.0"/>
								</asQualifiedEntity>
							</xsl:if>
								...
								<asLocatedEntity classCode="LOCE">
									<location classCode="COUNTRY" determinerCode="INSTANCE">
										...
										<code code="{sendercountrycode}" codeSystem="{$OidISOCountry}"
											  DisplayName="Sender Country Code"/>
									</location>
								</asLocatedEntity>
							</xsl:if>
						</assignedPerson>
...
E2B(R3): element "role"  -->
							...
							<kindOfProduct classCode="MMAT" determinerCode="KIND">
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when
													test="(string-length(krparentmedicalproductversion) > 0 and string-length(krparentmedicalproductid) > 0)">
												...
												<code code="{krparentmedicalproductid}" codeSystem="{$OIdKRDrugForeign}"
													  codeSystemVersion="{krparentmedicalproductversion}"/>
											</xsl:when>
											<xsl:when test="(string-length(krparentmedicalproductid) > 0)">
												<xsl:comment>D.10.8.r.1.KR.1b: Medicinal Product ID</xsl:comment>
												<code code="{krparentmedicalproductid}"
													  codeSystem="{$OIdKRDrugDomestic}"/>
											</xsl:when>
											<xsl:when
													test="string-length(parentmpidversion) > 0 and string-length(parentmpid) > 0">
												<xsl:comment>D.10.8.r.2a: MPID Version Date / Number</xsl:comment>
												<xsl:comment>D.10.8.r.2b: Medicinal Product Identifier (MPID)
												</xsl:comment>
												<code code="{parentmpid}" codeSystem="MPID"
													  codeSystemVersion="{parentmpidversion}"/>
											</xsl:when>
											<xsl:when
													test="string-length(parentphpidversion) > 0 and string-length(parentphpid) > 0">
	...
	<xsl:template match="parenteudrugsubstanceinfo" mode="EMA-parent-past-drug-substance-info">
		...
		<xsl:comment>Parent Past Drug Therapy - Substance / Specified Substance Identifier and Strength - (<xsl:value-of
				select="$positionParPastDrugSubInfo"/>)
		</xsl:comment>
					...
					<xsl:element name="numerator">
						...
						<xsl:attribute name="unit">
							<xsl:call-template name="getMapping">
								...
								<xsl:with-param name="code" select="parentdrgsubstancestrengthunit"/>
							</xsl:call-template>
						</xsl:attribute>
					</xsl:element>
			...
			<xsl:if test="(string-length(parentdrgsubstancename) > 0) or (string-length(parentdrgsubstancetermid) > 0 and string-length(parentdrgsubstancetermidversion) > 0)">
				<ingredientSubstance classCode="MMAT" determinerCode="KIND">
					<xsl:if test="string-length(parentdrgsubstancetermid) > 0 and string-length(parentdrgsubstancetermidversion) > 0">
						<xsl:comment>D.10.8.r.EU.r.2a: Substance / Specified Substance TermID Version Date / Number
						</xsl:comment>
						<xsl:comment>D.10.8.r.EU.r.2b: Substance / Specified Substance TermID</xsl:comment>
						<code code="{parentdrgsubstancetermid}" codeSystemVersion="{parentdrgsubstancetermidversion}"
							  codeSystem="{$oidMessageType}"/>
					</xsl:if>
		...
		<xsl:if test="string-length($value) > 0">
			...
			<xsl:choose>
				<xsl:when test="$isNullFlavourGender = 'yes'">
					...
					<administrativeGenderCode nullFlavor="{$NullFlavourWOSqBrcktGender}" codeSystem="{$oidGenderCode}"/>
				</xsl:when>
				<xsl:otherwise>
							...
							<kindOfProduct classCode="MMAT" determinerCode="KIND">
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when
													test="(string-length(krpatientmedicalproductid) > 0 and string-length(krpatientmedicalproductversion) > 0)">
												...
												<code code="{krpatientmedicalproductid}"
													  codeSystem="{$OIdKRDrugForeign}"
													  codeSystemVersion="{krpatientmedicalproductversion}"/>
											</xsl:when>
											...
											<xsl:when
													test="string-length(patientmpidversion) > 0 and string-length(patientmpid) > 0">
											...
											<xsl:when
													test="string-length(patientphpidversion) > 0 and string-length(patientphpid) > 0">
	...
	<xsl:template match="patientdrugsubstanceinfo" mode="EMA-patient-past-drug-substance-info">
		...
		<xsl:comment>Patient Past Drug Therapy - Substance / Specified Substance Identifier and Strength -
			(<xsl:value-of select="$positionPatPastDrugSubInfo"/>)
		</xsl:comment>
		<ingredient classCode="ACTI">
				...
				<quantity>
					...
					<xsl:element name="numerator">
						...
						<xsl:attribute name="unit">
							<xsl:call-template name="getMapping">
								<xsl:with-param name="type">UCUM</xsl:with-param>
								<xsl:with-param name="code" select="patientdrgsubstancestrengthunit"/>
							</xsl:call-template>
						</xsl:attribute>
					</xsl:element>
			...
			<xsl:if test="(string-length(patientdrgsubstancename) > 0) or (string-length(patientdrgsubstancetermid) > 0 and string-length(patientdrgsubstancetermidversion) > 0)">
				<ingredientSubstance classCode="MMAT" determinerCode="KIND">
					<xsl:if test="string-length(patientdrgsubstancetermid) > 0 and string-length(patientdrgsubstancetermidversion) > 0">
						<xsl:comment>D.8.r.EU.r.2a: Substance / Specified Substance TermID Version Date / Number
						</xsl:comment>
						<xsl:comment>D.8.r.EU.r.2b: Substance / Specified Substance TermID</xsl:comment>
						<code code="{patientdrgsubstancetermid}" codeSystemVersion="{patientdrgsubstancetermidversion}"
							  codeSystem="{$oidMessageType}"/>
					</xsl:if>
	...
	<xsl:template match="reaction">
		...
		<xsl:comment>E.i: REACTION(S)/EVENT(S) - (<xsl:value-of select="$positionevents"/>)
		</xsl:comment>
		<subjectOf2 typeCode="SBJ">
			<observation moodCode="EVN" classCode="OBS">
				...
				<xsl:if test="string-length(reactionmeddracode) > 0 or string-length(primarysourcereactionnative) > 0">
					...
					<value xsi:type="CE">
						...
						<xsl:if test="string-length(primarysourcereactionnative) > 0">
							...
							<xsl:comment>E.i.1.1b: Reaction / Event as Reported by the Primary Source Language
							</xsl:comment>
							<originalText>
				...
				<xsl:if test="string-length(primarysourcereaction) > 0">
					<xsl:comment>E.i.1.2: Reaction / Event as Reported by the Primary Source for Translation
					</xsl:comment>
					...
					<outboundRelationship2 typeCode="PERT">
						<observation moodCode="EVN" classCode="OBS">
							...
							<value xsi:type="CE" code="{termhighlighted}" codeSystem="{$oidTermHighlighted}"
								   codeSystemVersion="{$emaoidEi31TermHighlightedCLVersion}"/>
						</observation>
					</outboundRelationship2>
				</xsl:if>
					...
					<outboundRelationship2 typeCode="PERT">
						<observation moodCode="EVN" classCode="OBS">
							...
							<value xsi:type="CE" code="{reactionoutcome}" codeSystem="{$oidOutcome}"
								   codeSystemVersion="{$emaoidEi7OutcomeCLVersion}"/>
						</observation>
					</outboundRelationship2>
				</xsl:if>
					...
					<instanceOfKind classCode="INST">
						<kindOfProduct classCode="MMAT" determinerCode="KIND">
							<xsl:if test="string-length(krdrugmedicalproductid) > 0 and string-length(krdrugmedicalproductversion) > 0">
								...
								<code code="{krdrugmedicalproductid}" codeSystem="{$OIdKRDrugForeign}"
									  codeSystemVersion="{krdrugmedicalproductversion}"/>
							</xsl:if>
							<xsl:if test="string-length(krdrugmedicalproductid) > 0 and string-length(krdrugmedicalproductversion) =0">
				...
				<ingredientSubstance classCode="MMAT" determinerCode="KIND">
					<xsl:choose>
						<xsl:when test="(string-length(krsubstanceid) > 0 and string-length(krsubstanceversion) > 0)">
							...
							<code code="{krsubstanceid}" codeSystem="{$OIdKRSubstanceForeign}" codeSystemVersion="{krsubstanceversion}"/>
						</xsl:when>
						<xsl:when test="(string-length(krsubstanceid) > 0)">
							<xsl:comment>G.k.2.3.r.1.KR.1b: Substance ID</xsl:comment>
							<code code="{krsubstanceid}" codeSystem="{$OIdKRSubstanceDomestic}"/>
						</xsl:when>
						<xsl:when
								test="string-length(activesubstancetermid) > 0 and string-length(activesubstancetermidversion) > 0">
							...
							<code code="{activesubstancetermid}" codeSystem="TBD-Substance"
								  codeSystemVersion="{activesubstancetermidversion}"
								  displayName="Ingredient Information"/>
						</xsl:when>
					</xsl:choose>
	...
	<xsl:template match="drugindications" mode="EMA-drug-indication">
		<xsl:if test="string-length(drugindicationprimarysource) > 0 or string-length(drugindicationmeddracode) > 0 or string-length(drugindicationmeddraversion) > 0">
			<xsl:variable name="positionDrugInd">
				<xsl:value-of select="position()"/>
			</xsl:variable>
			<xsl:variable name="isNullFlavourDrugIndication">
				<xsl:call-template name="isNullFlavour">
					<xsl:with-param name="value" select="drugindicationprimarysource"/>
				</xsl:call-template>
			</xsl:variable>
			...
			<inboundRelationship typeCode="RSON">
				<observation moodCode="EVN" classCode="OBS">
					...
					<xsl:choose>
						<xsl:when
								test="(string-length(drugindicationmeddracode) > 0 and string-length(drugindicationmeddraversion) > 0)">
							<xsl:comment>G.k.7.r.2a: MedDRA Version for Indication</xsl:comment>
							<xsl:comment>G.k.7.r.2b: Indication for use in the case</xsl:comment>
							<value xsi:type="CE" code="{drugindicationmeddracode}" codeSystem="{$oidMedDRA}"
								...
								<xsl:choose>
									<xsl:when test="$isNullFlavourDrugIndication = 'yes'">
										<xsl:variable name="NullFlavourWOSqBrcktDrugIndication">
											<xsl:call-template name="getNFValueWithoutSqBrckt">
												<xsl:with-param name="nfvalue" select="drugindicationprimarysource"/>
											</xsl:call-template>
										</xsl:variable>
									...
									<xsl:when test="(string-length(drugindicationprimarysource) > 0)">
										<xsl:comment>G.k.7.r.1: Indication as Reported by the Primary Source
										</xsl:comment>
										<originalText>
											<xsl:value-of select="drugindicationprimarysource"/>
										</originalText>
									</xsl:when>
								</xsl:choose>
							</value>
									...
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when
												...
												<xsl:comment>G.k.4.r.9.1: Pharmaceutical form (Dosage form)
												</xsl:comment>
												<xsl:comment>G.k.4.r.9.2a: Pharmaceutical Dose Form TermID Version Date
													/ Number
												</xsl:comment>
				...
				<causalityAssessment classCode="OBS" moodCode="EVN">
						...
						<xsl:when test="string-length(whodrugassessmentresult) > 0">
							...
							<xsl:choose>
								<xsl:when test="$isNullFlavourGk9i2r3KR1 = 'yes'">
									...
									<value xsi:type="CE" nullFlavor="{$NullFlavourWOSqBrcktGk9i2r3KR1}" displayName="WHO-UMC Result of Assessment"/>
								</xsl:when>
								<xsl:when test="(string-length(whodrugassessmentresult) > 0)">
									<xsl:comment>G.k.9.i.2.r.3.KR.1: WHO-UMC Result of Assessment</xsl:comment>
									<value xsi:type="CE" code="{whodrugassessmentresult}" codeSystem="{$oidMFDSWHOResult}" codeSystemVersion="1.0" displayName="WHO-UMC Result of Assessment"/>
								</xsl:when>
							</xsl:choose>
						...
						<xsl:when test="string-length(krdrugassessmentresult) > 0">
							...
							<xsl:choose>
								<xsl:when test="$isNullFlavourGk9i2r3KR2 = 'yes'">
									...
									<value xsi:type="CE" nullFlavor="{$NullFlavourWOSqBrcktGk9i2r3KR2}" displayName="KRCT Result of Assessment"/>
								</xsl:when>
								<xsl:when test="(string-length(krdrugassessmentresult) > 0)">
									<xsl:comment>G.k.9.i.2.r.3.KR.2: KRCT Result of Assessment</xsl:comment>
									<value xsi:type="CE" code="{krdrugassessmentresult}" codeSystem="{$oidMFDSKRCTResult}" codeSystemVersion="1.0" displayName="KRCT Result of Assessment" />
								</xsl:when>
							</xsl:choose>
					...
					<xsl:choose>
						...
						<xsl:when test="string-length(krdrugassessmentmethod) > 0">
							...
							<methodCode code="{krdrugassessmentmethod}" codeSystem="{$oidMFDSKRMethod}" codeSystemVersion="1.0" displayName="KR Method of Assessment"/>
						</xsl:when>
			...
			<component1 typeCode="COMP">
				<observationEvent moodCode="EVN" classCode="OBS">
					<code code="{$Comment}" codeSystem="{$oidObservationCode}"
						  codeSystemVersion="{$emaObservationCLVersion}"/>
					<value xsi:type="ED" mediaType="text/plain">
						<xsl:value-of select="substring(reportercomment,1,20000)"/>
					</value>
					<author typeCode="AUT">
						<assignedEntity classCode="ASSIGNED">
							<code code="{$SourceReporter}" codeSystem="{$oidAssignedEntityRoleCode}"
			...
			<component1 typeCode="COMP">
				<observationEvent moodCode="EVN" classCode="OBS">
					<code code="{$Comment}" codeSystem="{$oidObservationCode}"
						  codeSystemVersion="{$emaObservationCLVersion}"/>
					<value xsi:type="ED" mediaType="text/plain">
						<xsl:value-of select="substring(sendercomment,1,20000)"/>
					</value>
					<author typeCode="AUT">
						<assignedEntity classCode="ASSIGNED">
		...
		<xsl:if test="string-length(krobservestudytype) > 0">
			...
			<subjectOf2 typeCode="SUBJ">
				<investigationCharacteristic classCode="OBS" moodCode="EVN">
					...
					<value xsi:type="CE" code="{krobservestudytype}" codeSystem="{$OidKRObservedStudy}"
